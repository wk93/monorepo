import type { PermissionItem } from "../api/usePermissionListQuery";
...
const PermissionList: React.FC<Props> = ({ items }) => {
  return (
    <nav aria-label="Directory">
      {items.map((category) => (
        <div key={category.name} className="relative">
          <div className="sticky top-0 z-10 border-y border-t-gray-100 border-b-gray-200 bg-gray-50 px-8 py-3.5 text-sm font-semibold text-gray-900">
            <h3 className="relative">{category.name}</h3>
          </div>
          <ul role="list" className="divide-y divide-gray-100">
            {category.items.map((item) => (
              <li key={item.key} className="flex gap-x-4 px-3 py-5">
